...
public class LicenseOcrUtils extends PaddleOcrUtils
{
    ...
    {
        String licenseCode = "";
        Pattern pattern = Pattern.compile(".*统一社会信用代码.(\\s{18})");
    ...
    private static String licenseNumber(String trim)
    {
        ...
        Pattern pattern = Pattern.compile(".*证照编号.(\\s{18})");
        Matcher matcher = pattern.matcher(trim);
        if (matcher.find())
        {
    ...
    private static String licenseEnterpriseName(String trim)
    {
        ...
        Pattern pattern = Pattern.compile(".*名称.(\\s*)注册资本");
        Matcher matcher = pattern.matcher(trim);
        if (matcher.find())
        {
    ...
    private static String licenseEstablishDate(String trim)
    {
        ...
        if (matcher.find())
        {
            licenseEstablishDate = matcher.group(1);
        }
        else
        {
    ...
    private static String[] licenseOperationPeriod(String trim)
    {
        ...
        Pattern pattern = Pattern.compile(".*成立日期(\\d{4})年(\\d{2})月(\\d{2})日至(\\d{4})年(\\d{2})月(\\d{2})日");
        Matcher matcher = pattern.matcher(trim);
        if (matcher.find())
        {
    ...
    private static Map<String, String> invokeLLM(String trim)
    {
        ...
        Pattern pattern = Pattern.compile("经营范围.([\u4e00-\u9fa5]*)\\s*住所.([\u4e00-\u9fa5]*)\\s*法定代表人.([\u4e00-\u9fa5]*)");
