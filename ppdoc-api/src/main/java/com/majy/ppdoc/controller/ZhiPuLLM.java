...
public class ZhiPuLLM
{
    ...
    public String zhiPuExtractInfo(@RequestBody String requestBody) throws Exception
    {
        ...
        return sseInvokeExtractInfo(ocrTextValue, fieldsValue);
    }
    ...
    public String zhiPuGenerateSummary(@RequestBody String requestBody)
    {
        System.setProperty("org.slf4j.simpleLogger.logFile", "System.out");
        // 1. sse-invoke调用模型，使用标准Listener，直接返回结果
        System.out.println("前端传回的摘要选项和OCR结果：" + requestBody);
        // 创建一个 JSONObject 对象
        JSONObject jsonObject = new JSONObject(requestBody);
        ...
        return sseInvokeGenerateSummary(ocrTextValue,summaryOptionValue);
    }
...
//                .tools(chatToolList) //删除工具列表，不传递任何工具信息
     ...
     * 通过sse调用，生成摘要
     */
    public static String sseInvokeGenerateSummary(String ocr_result, String summaryType)
    {
        List<ChatMessage> messages = new ArrayList<>();
