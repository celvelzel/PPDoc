  ...
  methods: {
    ...
    handleLicenseSelect(index, row) {
      this.$confirm('确认选择该记录?', '提示', {
        ...
        console.log(index, row);
        // 触发自定义事件，并将身份证ID作为参数传递给父组件
        this.$emit('select-license', row.license_id, row.license_code, row.license_enterprise_name, "defendant");
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消选择'
    ...
    highLightIdCardRow({row}) {
      if (row.card_number == this.caseInfoForm.defendant_id) {
    ...
    highLightLicenseRow({row}) {
      if (row.license_code == this.caseInfoForm.defendant_id) {
        console.log("行数据与组件状态中的数据相匹配")
        return 'highlight-row';
      }
    },
...
<template>
  <div v-if="this.caseInfoForm.defendant_type == '个人'">
    <el-row :gutter="20">
      <el-col span="11">
        <h1>起诉状预览</h1>
        <!-- PDF预览组件-->
  ...
  <div v-else-if="this.caseInfoForm.defendant_type != '个人'">
    <el-row :gutter="20">
      <el-col span="10">
        <h1>起诉状预览</h1>
        <!-- PDF预览组件-->
